...
import {
    ...
    Menu, X, CarFront, Sparkles, Search, Bell, Activity, ChevronRight
} from 'lucide-react';
...
export default function AdminLayout({ children, title }: AdminLayoutProps) {
    ...
    return (
        <div className="min-h-screen bg-slate-900 text-slate-100 flex font-sans selection:bg-indigo-600 selection:text-white">
            {/* Mobile Header */}
            <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-slate-950/90 backdrop-blur-md text-white px-4 h-16 flex items-center justify-between border-b border-slate-800">
                ...
                <div className="flex items-center gap-2 font-black text-lg">
                    ...
                    <span>Rent<span className="text-indigo-400">Bohol</span> Admin</span>
                </div>
                <div className="w-9" />
            </div>
            ...
            <div className="flex-1 lg:ml-64 min-h-screen bg-slate-900 flex flex-col pt-16 lg:pt-0">
                {/* Top Desktop Bar */}
                <header className="hidden lg:flex h-16 bg-slate-950/60 backdrop-blur-md border-b border-slate-800/80 px-8 items-center justify-between sticky top-0 z-20">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2 px-3 py-1 bg-indigo-500/10 border border-indigo-500/30 text-indigo-300 rounded-full text-[11px] font-bold">
                    ...
                    {title && (
                        <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-5">
                            ...
                            <div className="flex items-center gap-2">
                                <span className="px-3 py-1.5 bg-amber-400/10 border border-amber-400/30 text-amber-300 rounded-xl text-xs font-extrabold flex items-center gap-1.5 shadow-sm">
        ...
        <div className="flex flex-col h-full">
            ...
            <div className="p-4 border-t border-slate-800/80 bg-slate-950/60">
                <div className="flex items-center gap-3 mb-3 p-2 rounded-xl bg-slate-900 border border-slate-800">
                    <div className="w-8 h-8 rounded-lg bg-indigo-600 text-white font-extrabold text-xs flex items-center justify-center">
                        {auth.user?.name?.[0] || 'A'}
                    </div>
                    <div className="flex-1 min-w-0">
                        <p className="text-xs font-bold text-white truncate">{auth.user?.name}</p>
                        <p className="text-[10px] font-semibold text-indigo-400 truncate">Super Admin</p>
                    </div>
                </div>
                <Link
                    href="/logout"
